...
The [ISO 3166 part 1 2-letter codes](ValueSet-iso-3166-2-vs.html) and [sans Uzbekistan](ValueSet-iso-3166-2-vs-sans-uz.html) value sets no longer declare the [ISO 3166 two-letter supplement](CodeSystem-iso3166-two-letter-cs.html) as required through `valueset-supplement`. A required supplement can only be satisfied when the base code system is loaded locally or the value set has already been expanded; `urn:iso:std:iso:3166` is served only by the terminology server and a value set from a dependency package is never expanded, so validators rejected every instance bound to these value sets with `Required supplement not found`. This surfaced on `example-hepatitis-patient` in uz.dhp.integrations under IG Publisher 2.3.4, which fixed the NullPointerException that had been hiding it. The supplement itself is unchanged and is still forwarded to the terminology server, so the Uzbek and Russian designations continue to apply.
